...
static void build_lobby_message(Message *out_msg, const GameSnapshot *snapshots,
                                uint8_t count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    out_msg->payload.lobby_update.games[i]
        .owner_name[sizeof(out_msg->payload.lobby_update.games[i].owner_name) -
                    1] = '\0';
    out_msg->payload.lobby_update.games[i].players_connected =
        snapshots[i].players_connected;
  }
}
...
static void client_set_playing(int socket, bool playing, uint32_t game_id) {
  ...
  for (int i = 0; i < MAX_CLIENTS; i++) {
    if (clients[i].is_active && clients[i].socket == socket) {
      ...
      printf(
          "[Handler] client_set_playing: socket %d, playing=%d, game_id=%d\n",
...
void handle_message(int client_socket, Message *msg) {
  ...
  case MSG_JOIN_DECISION:
    handle_join_decision(client_socket, msg->payload.join_decision.accepted,
                         msg->payload.join_decision.game_id);
    break;
  case MSG_POST_GAME_OPTIONS:
    handle_post_game_decision(
        client_socket, msg->payload.post_game_options.game_id,
...
void handle_create_game(int client_socket) {
  if (client_is_playing(client_socket)) {
    ...
    err.type = MSG_ERROR;
    snprintf(err.payload.error.error_message,
             sizeof(err.payload.error.error_message), "Already in a game");
    send_message(client_socket, &err);
    return;
  }
  ...
  if (game_manager_create_game(game_manager, client_socket, username,
    ...
    err.type = MSG_ERROR;
    snprintf(err.payload.error.error_message,
             sizeof(err.payload.error.error_message), "Failed to create game");
    send_message(client_socket, &err);
    return;
  }
...
void handle_join_request(int client_socket, uint32_t game_id) {
  ...
  int ret =
      game_manager_join_game(game_manager, game_id, client_socket, username);
  if (ret == -1) {
    Message err = {0};
    err.type = MSG_ERROR;
    snprintf(err.payload.error.error_message,
  ...
  if (ret == 1) {
    ...
    if (owner_sock != -1) {
      ...
      snprintf(req.payload.join_request.requesting_player_name,
               sizeof(req.payload.join_request.requesting_player_name), "%s",
               username);
      ...
      printf("[Handler] Sent join request to owner (socket %d) for game %u "
             "from %s\n",
...
void handle_join_decision(int client_socket, bool accepted, uint32_t game_id) {
  ...
  int ret = game_manager_join_decision(game_manager, game_id, accepted ? 1 : 0,
                                       &assigned_role);
  if (ret < 0) {
    Message err = {0};
    err.type = MSG_ERROR;
  ...
  if (accepted) {
    ...
    if (game_manager_get_game_state(game_manager, game_id, &state) == 0) {
      ...
      snprintf(state_x.payload.game_state.message,
               sizeof(state_x.payload.game_state.message), "Game started");
...
void handle_move(int client_socket, Message *msg) {
  ...
  int ret = game_manager_move(game_manager, game_id, client_socket, row, col,
                              &finished, &winner);
  if (ret < 0) {
    Message err;
    err.type = MSG_ERROR;
  ...
  if (finished) {
    ...
    state_x.payload.game_state.state_data.player_role = PLAYER_X;
    snprintf(state_x.payload.game_state.message,
             sizeof(state_x.payload.game_state.message), "Board updated");
...
void handle_post_game_decision(int client_socket, uint32_t game_id,
                               bool wants_to_continue) {
  printf("[Handler] Post-game decision: socket=%d, game=%u, wants=%d\n",
         client_socket, game_id, wants_to_continue);

  bool both_decided;
  int ret =
      game_manager_post_game_decision(game_manager, game_id, client_socket,
                                      wants_to_continue ? 1 : 0, &both_decided);
  if (ret < 0) {
    Message err = {0};
    err.type = MSG_ERROR;
    snprintf(err.payload.error.error_message,
  ...
  if (both_decided) {
    ...
    if (state.state == GAME_WAITING || state.state == GAME_IN_PROGRESS) {
      int x_sock, o_sock;
      if (game_manager_get_players(game_manager, game_id, &x_sock, &o_sock) ==
          0) {
        if (!wants_to_continue) {
          if (x_sock == client_socket)
            x_sock = -1;
          if (o_sock == client_socket)
            o_sock = -1;
        }
        if (o_sock == -1) {
          if (x_sock != -1) {
          ...
          if (game_manager_get_game_state(game_manager, game_id, &new_state) ==
              0) {
            Message state_x, state_o;
            memset(&state_x, 0, sizeof(state_x));
            memset(&state_o, 0, sizeof(state_o));
...
void handle_set_username(int client_socket, const SetUsernamePayload *payload) {
  ...
  for (int i = 0; i < MAX_CLIENTS; i++) {
    if (clients[i].is_active && clients[i].socket == client_socket) {
      strncpy(clients[i].username, payload->username,
...
void handle_leave_game(int client_socket, uint32_t client_provided_game_id) {
  uint32_t target_game_id = client_provided_game_id;

  pthread_mutex_lock(&clients_mutex);
  for (int i = 0; i < MAX_CLIENTS; i++) {
    if (clients[i].is_active && clients[i].socket == client_socket) {
      if (clients[i].current_game_id != -1) {
        target_game_id = (uint32_t)clients[i].current_game_id;
      }
      break;
    }
  }
  ...
  printf("[Handler] Received leave request. Socket: %d, Target Game: %u\n",
         client_socket, target_game_id);

  int ret =
      game_manager_leave_game(game_manager, target_game_id, client_socket);

  game_manager_cleanup_creator_games(game_manager, client_socket, 0);

  if (ret < 0) {
    fprintf(stderr,
            "[Handler] Failed to leave specific game %u for socket %d (but "
            "cleaned up any orphans)\n",
            target_game_id, client_socket);
  } else {
    printf("[Handler] Player left game %u\n", target_game_id);
  }

  client_set_playing(client_socket, false, UINT32_MAX);
  broadcast_lobby_update();
}
