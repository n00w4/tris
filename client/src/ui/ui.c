...
typedef enum { SETTINGS_NAV, SETTINGS_EDIT } SettingsMode;
...
typedef struct {
  ...
  int menu_selection;
  int last_game_winner; // winner of the last game (PLAYER_X, PLAYER_O,
                        // RESULT_DRAW, RESULT_NONE)
} UIState;
...
void ui_stop(void) { atomic_store(&ui_running, 0); }
...
static bool ensure_network_connected(UIState *state, queue_t *to_net,
                                     queue_t *from_net) {
  if (state->is_connected && state->net_ctx &&
      network_is_connected(state->net_ctx)) {
    return true;
  }
  if (state->net_ctx) {
  ...
  state->net_ctx = network_start(state->config.username, state->config.ip,
                                 atoi(state->config.port), from_net, to_net);
  if (!state->net_ctx) {
    strncpy(state->error_message, "Failed to connect to server",
  ...
  if (!network_is_connected(state->net_ctx)) {
    ...
    network_destroy(state->net_ctx);
    state->net_ctx = NULL;
    strncpy(state->error_message, "Could not connect to server",
            sizeof(state->error_message) - 1);
    return false;
  }
...
// menu action helper
static void do_menu_action(UIState *state, int action, queue_t *to_net,
                           queue_t *from_net) {
  switch (action) {
  case 0: // create game
...
static void draw_main_menu(UIState *state) {
  ...
  const char *items[] = {"Create game", "Join game", "Settings", "Help",
                         "Quit"};
  int item_count = 5;
  ...
  mvprintw(mid_y + 2 + item_count + 1, (COLS - 40) / 2,
...
static void draw_lobby(UIState *state) {
  ...
  for (int i = 0; i < state->lobby_count && i < 10; i++) {
    mvprintw(
        y++, 4, "%d - ID: %u - Owner: %s - (%d players) - STATUS: %s", i + 1,
        state->lobby_games[i].game_id, state->lobby_games[i].owner_name,
  ...
  mvprintw(LINES - 2, 2,
           "Press number to join, 'r' to refresh, ESC to go back");
  draw_status_bar(state);
  refresh();
}
...
static void draw_game_board(UIState *state) {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      int x =
          start_x + j * 6 + 2; // horizontal center (2 spaces from left border)
...
static void draw_join_request(UIState *state) {
  clear();
  mvprintw(LINES / 2 - 2, (COLS - 30) / 2, "Join request from: %s",
           state->join_requester_name);
...
static void draw_result_message(UIState *state, bool play_again) {
  ...
  if (state->last_game_winner == RESULT_DRAW) {
    snprintf(display_msg, sizeof(display_msg), "DRAW! %s",
...
static void draw_settings(UIState *state) {
  ...
  if (state->settings_active_field == 0 &&
      state->settings_mode == SETTINGS_NAV) {
    attron(A_REVERSE);
  }
  ...
  y += 2;
  if (state->settings_active_field == 1 &&
      state->settings_mode == SETTINGS_NAV) {
    attron(A_REVERSE);
  }
  mvprintw(y, x, "IP: ");
  ...
  y += 2;
  if (state->settings_active_field == 2 &&
      state->settings_mode == SETTINGS_NAV) {
    attron(A_REVERSE);
  }
  mvprintw(y, x, "Port: ");
  ...
  if (state->settings_mode == SETTINGS_EDIT) {
    mvprintw(LINES - 2, 2,
             "Editing: type and press ENTER to confirm, ESC to cancel");
  } else {
...
static void draw_help(UIState *state) {
  ...
  mvprintw(y++, 4,
           "- Use ARROW KEYS to navigate menus and move cursor on board.");
  mvprintw(y++, 4, "- Press ENTER to select an option or place a move.");
  mvprintw(y++, 4,
           "- Press ESC to go back (except main menu where ESC exits).");
...
// input handling
static void handle_keyboard(UIState *state, int ch, queue_t *to_net,
                            queue_t *from_net) {
  if (state->error_message[0] != '\0') {
    if (ch == '\n' || ch == ' ' || ch == 27) {
  ...
  case SCREEN_SETTINGS: {
    if (state->settings_mode == SETTINGS_EDIT) {
      if (ch == '\n') {
        ...
        case 0:
          snprintf(state->config.username, sizeof(state->config.username), "%s",
                   state->settings_edit_buf);
          break;
        case 1:
          snprintf(state->config.ip, sizeof(state->config.ip), "%s",
                   state->settings_edit_buf);
          break;
        case 2:
          snprintf(state->config.port, sizeof(state->config.port), "%s",
                   state->settings_edit_buf);
          break;
        }
        state->settings_mode = SETTINGS_NAV;
...
static void handle_event(UIState *state, UIEvent *ev) {
  ...
  case UI_EVENT_GAME_OVER:
    ...
    state->last_game_winner =
        ev->data.game_over.winner; // store winner for YOU WIN/LOSE display
  ...
  case UI_EVENT_LOBBY_UPDATE:
    ...
    for (int i = 0; i < state->lobby_count; i++) {
      ...
      state->lobby_games[i]
          .owner_name[sizeof(state->lobby_games[i].owner_name) - 1] = '\0';
      state->lobby_games[i].players_connected =
          ev->data.lobby_update.games[i].players_connected;
    }
    break;
  case UI_EVENT_ERROR:
  ...
  case UI_EVENT_JOIN_REQUEST:
    strncpy(state->join_requester_name,
            ev->data.join_request.requesting_player_name,
