...
static void thread_exit_error(network_ctx *ctx, const char *user_msg) {
  ...
  if (ev) {
    ev->type = UI_EVENT_ERROR;
    snprintf(ev->data.error.error_message, sizeof(ev->data.error.error_message),
             "%s", user_msg);
...
static void *network_thread_func(void *arg) {
  ...
  snprintf(set_user_msg.payload.set_username.username,
           sizeof(set_user_msg.payload.set_username.username), "%s",
           ctx->username);
    ...
    if (pret > 0 && (pfd.revents & POLLIN)) {
      ...
      if (r == -2) { // connection closed from server
        ...
        if (ev) {
          ev->type = UI_EVENT_ERROR;
          snprintf(ev->data.error.error_message,
                   sizeof(ev->data.error.error_message), "Receive error");
          queue_push(ctx->to_ui, ev);
        }
        break;
      }
